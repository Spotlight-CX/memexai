...
export function appendLinesAfterHeading(content: string, afterHeading: string, lines: string[]) {
  ...
  if (headingIndex < 0) {
    throw new MemexError("PATCH_HEADING_NOT_FOUND", `Heading not found: ${afterHeading}`)
  }
...
export function replaceExactText(content: string, match: string, replacement: string | string[]) {
  ...
  if (count === 0) throw new MemexError("PATCH_MATCH_NOT_FOUND", "Exact match not found")
  if (count > 1) throw new MemexError("PATCH_AMBIGUOUS_MATCH", `Ambiguous match (${count} occurrences)`)
