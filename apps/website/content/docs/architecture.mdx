...
| Tool | What it does |
| --- | --- |
| `memory_remember` | Feeds raw text to an inner model that decides what durable facts should be written or patched. |
| `memory_context` | Assembles retrieval context; internally calls `memory_find`, `memory_read`, and `memory_list` to build a grounded response with cited sources. |
...
MemexAI uses a cognitive architecture triad to structure memory at two levels. `shared/` holds schemas and rules; `user/` holds per-user instances.

```
shared/procedural.md   ← always injected: operator rules, tool policy
shared/semantic.md     ← schema: what facts to collect in user/profile.md
...
Every `shared/*` file and every `user/{userId}/*` file is injected into the prompt block automatically — no explicit path configuration needed. See [Cognitive Architecture](/docs/concepts/cognitive-architecture) for templates and examples.
...
Search starts with Postgres full-text search (`tsvector`) via `memory_find` — BM25 is always available with no configuration. When `GEMINI_API_KEY` is set and `MEMEX_SEARCH_MODE=auto` (the default), the service also generates pgvector embeddings and merges BM25 and vector candidates with Reciprocal Rank Fusion (RRF) into a single ranked list. Set `MEMEX_SEARCH_MODE=bm25` to force BM25-only. `memory_context` calls `memory_find` internally, so hybrid search is available to the agentic layer automatically whenever configured. BM25 remains the default; pgvector is an optional enhancement, not a separate vector database requirement.
