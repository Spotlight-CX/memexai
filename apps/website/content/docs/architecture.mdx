...
The v1 operator UX is API-level:
...
Enable the scheduler with `MEMEX_DREAM_ENABLED=true`. The database `dream_enabled` key remains the runtime master switch. There is no dedicated admin UI panel yet; operators inspect changed files through the existing file and revision views.
