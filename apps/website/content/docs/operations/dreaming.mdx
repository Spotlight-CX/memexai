...
Environment variable (disables the scheduler at startup):

```bash
MEMEX_DREAM_ENABLED=false
```

Runtime toggle without restarting the service:

<DocTabs items={['Admin UI', 'HTTP', 'CLI']} groupId="memex-interface" persist>
<DocTab value="Admin UI">
Open `/admin` → **Dreams** → toggle the **Dreaming enabled** switch to off. Takes effect on the next scheduler tick.
</DocTab>
<DocTab value="HTTP">
```http
...
<DocTabs items={['Admin UI', 'HTTP', 'CLI']} groupId="memex-interface" persist>
<DocTab value="Admin UI">
Open `/admin` → **Dreams** → find the user row → click **Pause**. The user is skipped on all future ticks until you click **Resume**.
</DocTab>
<DocTab value="HTTP">
```http
...
```bash
# Pause
memex-admin dream pause {userId}

# Resume
memex-admin dream unpause {userId}
```
</DocTab>
</DocTabs>
