...
![MemexAI admin observability context view with topology, hygiene, activity, top files, and slow calls](/images/admin-console/observability-context.png)
...
- Run raw file tools such as `memory_list`, `memory_read`, `memory_write`, and `memory_patch`
- Run memory subagent such as `memory_remember` and `memory_context` when a model is configured
...
Without a configured model, `memory_context` can still use `memory_find` (Postgres full-text search), or hybrid pgvector search when an embedding adapter is configured, while `memory_remember` returns `MODEL_NOT_CONFIGURED`.
...
The service exposes docs at `/v1/docs` — no auth required. This makes documentation accessible to agents and CLI tools running inside or alongside your MemexAI deployment:

```bash
# List available doc files
curl http://localhost:8080/v1/docs

# Read a specific doc as markdown
curl http://localhost:8080/v1/docs/concepts/cognitive-architecture.md

# From inside the container
docker exec memexai-memexai-1 wget -qO- http://localhost:8080/v1/docs
```

Docs are bundled from the `docs/` directory in the repo. Override the path with `MEMEX_DOCS_PATH=/your/docs/dir`.

The MemexAI website also serves all concept docs as clean markdown for LLMs and agents:

```bash
# Full docs index
curl https://memexai.space/llms.txt

# Any concept page as raw markdown
curl https://memexai.space/llms.mdx/docs/concepts/cognitive-architecture/content.md
curl https://memexai.space/llms.mdx/docs/concepts/memory-tools/content.md
```
