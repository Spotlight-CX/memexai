...
Dreaming is optional background memory consolidation. When enabled, the service periodically finds users whose memory files have changed, waits for a quiet grace period, then runs a consolidation pass that merges duplicates, clarifies fragmented notes, and records revisions with `actor='dream-agent'`.
...
The database `dream_enabled` config key is the runtime master switch. Admin endpoints under `/v1/admin/dream/*` let operators read and update the dream config, list user dream status, and pause a user. There is no separate admin UI panel yet; use the existing file and revision views to inspect the results.
