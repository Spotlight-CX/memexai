...
LLM-backed `memory_memorize` and memory subagent `memory_search` are configured on the service, not in the SDK.
...
For the smoothest first run, set `GEMINI_API_KEY` on the service so the memory subagent `memory_memorize` tool can decide what should become durable memory.
