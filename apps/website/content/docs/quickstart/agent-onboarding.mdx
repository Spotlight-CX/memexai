...
| --- | --- |
| Memory subagent mode or Raw file tools mode? | Memory subagent mode |
| Set up local Docker Compose? | Yes |

Memory subagent mode exposes the higher-level memory tools:

- `memory_remember`
- `memory_context`

Raw file tools mode is for advanced integrations and debugging. It may expose file-level tools such as `memory_list`, `memory_read`, `memory_write`, and `memory_patch`.
...
`memory_remember` accepts any durable text candidate — user input, tool results, assistant observations, task outputs, or session summaries. You control what gets passed to it and when.

There are two common paths:

| Path | Use when |
| --- | --- |
| Hot path | The current turn depends on memory, or the user expects an immediate save |
| Background path | Latency matters, or you want batching, dedupe, review, or a cheaper extractor |

Neither path is the only correct architecture. A hot-path `await memory.remember(...)` is right for explicit “remember this” turns. Background extraction is right when learning can happen after the response.

For tool-call learning, extract a compact insight before calling MemexAI:

```ts
for (const step of result.steps) {
  for (const toolResult of step.toolResults ?? []) {
    if (isPermanentError(toolResult.result)) {
      const insight = await extractInsight(toolResult) // app-owned policy
      if (insight) memory.remember({ text: insight }).catch(console.error)
    }
  }
}
```

For operational facts worth sharing across all users (SQL dialect quirks, API limitations), write to `shared/` by adding a routing entry in `shared/index.md` and enabling `MEMEX_SHARED_WRITE_MODE=rw` on the service. See [Memory Tools — Extraction patterns](/docs/concepts/memory-tools#extraction-patterns) for SDK examples.

| Framework | Recommended extraction point |
| --- | --- |
| Vercel AI SDK | `result.steps` or stream step events |
| OpenAI / Anthropic SDK | manual tool-call loop |
| LangChain / LangGraph | callback, runnable wrapper, or graph node |
| LlamaIndex | post-response workflow step |
| CrewAI | task-output extraction |
| Google ADK | native memory service lifecycle |
...
When the model calls `memory_remember`, MemexAI decides what durable facts are worth keeping, validates the requested virtual path, translates `user/...` to the current user's physical database path, writes the file, and stores a revision plus an access log. When the next model call uses `memory.getSystemPrompt(...)`, MemexAI injects shared memory and the current user's memory index into the system prompt so the model can search, read, and answer from the durable record.
...
- the service has `GEMINI_API_KEY` or another model provider when using `memory_remember`
...
Got it working? We're a small team and genuinely want to hear how it went — what clicked, what didn't, or what you built. [Join us on Slack →](https://join.slack.com/t/memexaispace/shared_invite/zt-3yy24alf6-t1wRQsErf09JViHww_qlGw) for quick help or to share feedback. If you found a bug or have a suggestion, [open an issue on GitHub](https://github.com/memexai/memexai).
