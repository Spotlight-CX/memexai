...
Read [How MemexAI Works](/docs/concepts/how-it-works) for the full request-to-storage-to-recall loop. For definitions of key terms (virtual path, physical path, operator, dreaming), see [Design Principles — Vocabulary](/docs/concepts/design-principles#vocabulary).
...
Pass both `system` and `tools` into your model call. Tools store and retrieve memory; the prompt block — the XML section MemexAI injects into your system prompt — is what puts stored memory into the model's context window on the next call (see [Prompt Block](/docs/concepts/prompt-block)).
...
<Cards>
  ...
  <Card title="Cognitive architecture" href="/docs/concepts/cognitive-architecture" description="Structure memory using the procedural/semantic/episodic triad — and understand why the shared/user two-level layout works." />
