...
**Pattern B — Org-level shared memory via deep paths**

For trusted deployments where agents share team or org-level context, use deep paths under `shared/` with `MEMEX_SHARED_WRITE_MODE=rw`:
