...
When this is enabled, `memory_write` and `memory_patch` can target `shared/**`, and the prompt block/tool descriptions tell the agent that shared memory is writable. Use it only for durable global knowledge. Keep private user facts, secrets, raw transcripts, and untrusted external content out of `shared/**`.
