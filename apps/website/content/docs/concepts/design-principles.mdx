...
BM25 and memory-link traversal cover most retrieval cases in practice. When you need semantic recall, optional pgvector hybrid search is available via `MEMEX_SEARCH_MODE=auto` — see [Docker service setup](/docs/quickstart/docker-service). In hybrid mode, `memory_search` and query-seeded `memory_smart_read` can use vector ranking, but the Markdown file remains the canonical memory record.
...
`memory_smart_read` returns the same files for the same query on the same data and search mode, every time.
