...
`memory_remember` reads `shared/user-memory.md` if it exists. That file can contain a routing table that tells the agent which file to use for which category of fact (preferences, family, health, visits). This gives your deployment deterministic write routing without hard-coding it in application code.
