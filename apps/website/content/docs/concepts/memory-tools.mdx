...
```python
# Most common: user message after a completed turn
await memory.remember(user_message)

# Tool result: pass the durable fragment, not a raw dump
await memory.remember("run_sql failed: ILIKE is not supported in this SQL dialect")

# Assistant observation or stated fact
await memory.remember(assistant_response)
```
</DocTab>
</DocTabs>
...
```python
# Typical assistant setup
system = await memory.get_system_prompt("You are a helpful assistant with durable user memory.")

# Call memory_remember and memory_context via framework adapter
# or directly:
await memory.remember("User prefers 2BHK in Indiranagar")
context = await memory.retrieve_context("apartment requirements")
```
</DocTab>
</DocTabs>
...
- [Cognitive architecture](/docs/concepts/cognitive-architecture) — how to structure `shared/` and `user/` files using the procedural/semantic/episodic triad
