...
description: The engineering decisions behind MemexAI — why markdown files, why agentic tools, why not git, why auth is structural, and why retrieval is single-shot.
---
...
MemexAI stores memory as Markdown text in Postgres. BM25 full-text search is the default retrieval layer — fast, reproducible, testable. pgvector (via `MEMEX_SEARCH_MODE=auto`) is optional semantic acceleration, not the source of truth. Both modes produce results with a `reason` field so you know exactly how each file was found.

This makes memory inspectable by humans and reproducible in tests.

## Why agentic tools, not host-app injection
...
2. **Agentic tools** — the model calls `memory_read`, `memory_list`, `memory_write` as needed.

Both are valid. Best systems often use both: agentic tools for model-driven decisions, direct writes for deterministic product events (e.g., an end-of-session summary triggered by your server code).

The advantage of agentic tools is that the model decides what to read based on the current context — the host app doesn't need to anticipate relevance. Every read and write is logged with a `tool_call_id`, so every memory operation is traceable to a specific LLM response. That traceability is what makes post-hoc debugging possible.
