...
- MemexAI LangChain adapter docs: Python service-mode usage imports `MemexAI`, `get_langchain_tools`, and `ChatGoogleGenerativeAI`; the adapter exposes all seven MemexAI tools, with `memory_remember` and `memory_context` recommended for most assistants.
...
- The code narrows the MemexAI wrapper set to `memory_remember` and `memory_context` for the smallest durable-memory loop.
...
- Post-run MemexAI SDK verification: `memory_list` reported 3 visible files and `memory_context("2BHK apartments near metro stations")` contained both `2BHK` and `metro`.
...
- This example depends on a live Gemini key for the app model and, when `memory_remember` is used, a MemexAI service configured with an LLM provider.
