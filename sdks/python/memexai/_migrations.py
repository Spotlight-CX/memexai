...
async def run_migrations(db: DbPool) -> None:
    await db.execute("""
        CREATE TABLE IF NOT EXISTS mx_migration (
          id TEXT PRIMARY KEY,
          applied_at TIMESTAMPTZ NOT NULL DEFAULT now()
    ...
    for migration in MIGRATIONS:
        rows = await db.query("SELECT id FROM mx_migration WHERE id = $1", migration["id"])
        if len(rows) > 0:
            continue
        ...
        async with db.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute("INSERT INTO mx_migration (id) VALUES ($1)", migration["id"])
